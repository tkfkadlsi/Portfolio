...
void ResourceManager::Play(const wstring& _key, SOUND_CHANNEL _channel)
{
	...
	m_pSoundSystem->playSound(ptSound->pSound, nullptr, false, &m_pChannel[(UINT)_channel]);
}
...
int ResourceManager::GetTime(SOUND_CHANNEL _channel)
{
	unsigned int time = 0;
	m_pChannel[(UINT)_channel]->getPosition(&time, FMOD_TIMEUNIT_MS);
	time -= 55;
	return time;
}

void ResourceManager::LoadChaebo(const wstring& _key, const wstring& _path)
{
	wstring* pChaebo = FindChaebo(_key);
	...
	pChaebo = new wstring(wss.str());

	m_mapChaebos.insert({_key, pChaebo});
}

wstring* ResourceManager::FindChaebo(const wstring& _key)
{
	map<wstring, wstring*>::iterator iter = m_mapChaebos.find(_key);

	if (iter == m_mapChaebos.end())
		return nullptr;
	return iter->second;
}
