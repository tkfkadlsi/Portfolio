...
public class BossRayWeapon : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        slime = transform.root.GetComponent<BossSlime>();
    ...
    private IEnumerator SizeUp(bool isX)
    {
        ...
        if (isX)
        {
            ...
            Vector3 endScale = new Vector3(transform.localScale.x, 3);
            while (t < lerpTime)
            {
        ...
        else if (!isX)
        {
            ...
            Vector3 endScale = new Vector3(3, transform.localScale.y);
            while (t < lerpTime)
            {
                transform.localScale = Vector3.Lerp(startScale, endScale, sizeUp_Curve.Evaluate(t / lerpTime));
